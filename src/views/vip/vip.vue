    ...
    <div class="vip_content">
      ...
      <div class="vip_pay">
        ...
        <div class="price_list">
          <div
            :class="['price_item', item.id == vipType.id ? 'active' : '']"
            v-for="(item, index) in userVipInfo.productResponseVos"
            :key="index"
            @click="selectVipType(item)"
          >
            <div class="vip_type">{{ vipTypes[item.productCode].name }}</div>
            <div class="c_price AlibabaPuHuiTi">
              ¥<span>{{ turnMoney(item.currentPrice) - 0 }}</span>
            </div>
            <div class="o_price" v-if="item.currentPrice < item.originalPrice">
              ¥<span>{{ turnMoney(item.originalPrice) - 0 }}</span>
            </div>
            <div class="per_day">¥{{ item.perDay }}/天</div>
          </div>
        </div>
        <div class="balance_time">
          <div class="time_out" v-if="userVipInfo.vipStatus == 0">
            新人折扣：<van-icon name="clock-o" /> <span style="font-weight: bold">{{ timeBalance }}</span> 后恢复原价
          </div>
          <div class="balance" @click="tocz">
            您有学币：{{ userVipInfo.balance }} <img src="@img/cz@2x.png" alt="" />
          </div>
        </div>
    ...
    <van-popup v-model:show="commentPop" round>
      <div class="comment_pop">
        ...
        <van-field
          ...
          autosize
          label=""
          placeholder="请输入您的评论，被审核通过后被优先展示在评论区（根据相关规定，您的评论将展示IP地址）"
        />
