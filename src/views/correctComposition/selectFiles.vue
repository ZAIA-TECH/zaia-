    ...
    <van-popup v-model:show="showPayPop" round position="bottom">
      <div class="pay_pop">
        ...
        <div class="pay_info">
          ...
          <div class="my_coin">
            <div class="my_coin_right">
              我的学币：<span>{{ turnMoney(payInfo.balance) }}</span> 个
            </div>
...
const toEditComposition = () => {
  ...
  router.replace({
    ...
    query: { ...route.query, orderNo, agentId: store.checkedAgents.map((val) => val.agentId).join(',') }
  })
}
...
const toFeedBack = () => {
  ...
  const documentTitle = `${grades[identity.userGrade].grade}${type.value == 'zh' ? '语文' : '英语'}-反馈列表`

  router.push({ path: './feedBackList', query: { ...route.query, documentTitle } })
}
