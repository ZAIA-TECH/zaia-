  ...
  <div class="correction_report">
    ...
    <div class="report_content"> 
      ...
      <div class="error_content" v-if="activeNav == 0">
        ...
        <div class="comment_bot" v-show="loadDone">
...
<script setup>
  ...
  import { parseHtml, fitUnitPx, countCharacters, countEnWords } from '@/utils/utils'
  ...
  const setScore = (res) => {
    ...
    if(isRecor) {
      ...
      lastScoreCollection.value = lastAnswer.Result.scoreCollection?.perspectiveScore || lastAnswer.Result.majorScore;
      lastScoreCollection.value.score = lastAnswer.Result.scoreCollection?.score
      lastScoreCollection.value.totalScore = lastAnswer.Result.totalScore
      for(let key in scoreNums.value) {
        scoreDiff.value[key] = {};
        scoreDiff.value[key].num = (scoreNums.value[key] - lastScoreCollection.value[key]).toFixed(1);
  ...
  const setDetailData = (res) => {
    ...
    console.log(JSON.parse(JSON.stringify(answerData)))
    ...
    res.data.orderContentPo.wordLength = countEnWords(res.data.orderContentPo.content);
  ...
  const comment = async(commentAi) => {
    const lastSores = isRecor ? {
      ...
      lastThemeExplicit: lastScoreCollection.value.themeExplicit,
      lastStructureStrict: lastScoreCollection.value.structureStrict,
      lastSentimentSincerity: lastScoreCollection.value.sentimentSincerity,
      lastEssayFluence: lastScoreCollection.value.essayFluence,
      lastGoodSent: lastScoreCollection.value.goodSent,

      lastTotalScore: lastScoreCollection.value.totalScore,
      lastWordScore: lastScoreCollection.value.wordScore,
      lastGrammarScore: lastScoreCollection.value.grammarScore,
      lastStructureScore: lastScoreCollection.value.structureScore,
      lastTopicScore: lastScoreCollection.value.topicScore,
    } : {}
    let aiInfo = {}
    aiCommentCollection.value = '';
    evaluateAi.value = 0;
    loadDone.value = false;
    await scoreOrderComment({
      stream: true,
      isRecor,
      dicDetailDesc: label.cnLiterary?.dicDetailDesc || label.enGrade?.dicDetailDesc,
      ...
      ...lastSores,
      callBack(aiRes) {
        aiRes.firstStream && (autoBot = true);
        if(!aiRes.done) {
          aiCommentCollection.value += aiRes.message;
          aiInfo = {...aiRes.aiInfo, prompt: aiRes.prompt};
        } else {
          loadDone.value = true;
          aiCommentCollection.value =  parse(aiCommentCollection.value);
          // props.activeNav == 2 && scrollToPos();
        }
        autoBot && scrollToPos();
      }
    })
    generation(aiInfo, isRecor ? 'reScoreOrder' : 'scoreOrder', aiCommentCollection.value)
  }
  ...
  const scrollToPos = (pos) => {
    ...
    botTimer = setTimeout(() => {
      window.scrollTo({
        behavior: 'smooth',
        top: !activeNav.value ? (pos || 10000000) : 0,
      });
    }, 50)
  } 

  const generation = async (aiInfo, sceneType, outPut) => {
    const data = {
      evaluateAi: 0,
      model: aiInfo.model,
      orderNo,
      ...
      requestId: aiInfo.request_id,
      sceneType,
      source: orderContent.value.content,
      sourceId: 0,
      tokensCost: aiInfo.usage.total_tokens,
  ...
  onMounted(async() => {
    ...
    document.addEventListener('click', unAutoBot, false)
    getOrderDetail()
  })

  onBeforeUnmount(() => {
    document.removeEventListener('click', unAutoBot, false)
  }) 
