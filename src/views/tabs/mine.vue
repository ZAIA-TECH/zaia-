  ...
  <div class="mine">
    ...
    <div @click="toVip" :class="{ vip_card: 1, de_vip_card: !store.isVip, unlogin_vip_card: !store.isLogin }">
      ...
      <div class="end_data" v-else>
        <template v-if="!userInfo.vipActivate"> 会员有效期，自开通后首次批改起算 </template>
        <template v-else> 有效期至：{{ userInfo.vipEndTime }} </template>
      </div>
      <div class="bi_num" v-if="store.isLogin">
        已持有<span>{{ store.isLogin ? turnMoney(userInfo.balance) : 0 }}</span
        >学币
    ...
    <div class="operate_list">
      <template v-for="(item, index) in operateList" :key="index">
        <div
          class="operate_item"
          @click="routeLink(item.route, item.query, item.method, item.outLink)"
          v-if="!item.hide"
        >
          <div class="item_name">
...
const operateList = ref([
  ...
  {
    name: '领会员',
    route: '/freeAccessCoin',
    icon: getAssetURL('vip_icon.png'),
    hideDot: !store.hideDot && store.isLogin
  },
  {
    name: '联系我们',
    route: '',
    icon: getAssetURL('serve_icon.png'),
    method: () => {
      showPop.value = true
      popType.value = 'serve'
    }
  },
  {
    name: '公众号',
    route: '',
    icon: getAssetURL('offical_icon.png'),
    method: () => {
      showPop.value = true
  ...
  },
  {
    name: '下载APP',
    route: '',
    icon: getAssetURL('app_icon.png'),
    outLink: 'https://a.app.qq.com/o/simple.jsp?pkgname=cn.zakj.zws'
  }
])
